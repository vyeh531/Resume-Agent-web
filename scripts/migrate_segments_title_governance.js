...
async function main() {
  ...
  try {
    ...
    await pool.query("CREATE INDEX IF NOT EXISTS idx_segments_canonical_title ON segments (canonical_title)");
    await pool.query("CREATE INDEX IF NOT EXISTS idx_segments_title_review_status ON segments (title_review_status)");
