...
Generated: 2026-06-02T23:27:12.212Z
...
- data_engineer (data, positions=18): top=12, flags=4, buried=5
...
- legal_compliance (legal_policy, positions=8): top=12, flags=1, buried=5
...
- operations (business_ops, positions=7): top=12, flags=6, buried=5
...
- hospitality_events (business_ops, positions=3): top=12, flags=8, buried=5
...
- education (education_research, positions=2): top=12, flags=0, buried=5
- other (other, positions=2): top=12, flags=5, buried=5
- social_services (healthcare_life_sciences, positions=2): top=12, flags=4, buried=5
...
### Flagged Current Top Rows
- #1 seg_21863 [not_role_safe] role=data_engineer,data_analyst target=data_engineer,data_analyst
...
- #7 seg_11416 [not_role_safe] role=universal target=data_analyst
  - title: 求职者技能列表未针对不同JD进行定制，缺少岗位要求的具体技术关键词，导致ATS匹配度低。
  - action: 逐一对照目标JD的技能要求，将岗位明确提及的工具（如pandas、NumPy、MySQL）添加至技能列表；多看同类JD可总结出普遍必备技能，提前纳入简历。
...
- #10 seg_17178 [no_role_signal] role=universal target=universal
...
### Buried Candidate Rows
- id=14866 score=17 role=supply_chain_logistics,project_program_management target=supply_chain_analyst,procurement_specialist,project_manager tags=missing_priority_keywords,keywords_only_in_skills,weak_target_role_alignment,low_hard_skill_match,low_jd_keyword_match
...
### Flagged Current Top Rows
- #6 seg_1570 [role_mismatch] role=machine_learning,data_analyst target=data_analyst
...
- #9 seg_22419 [not_role_safe] role=universal target=universal
  - title: 求职者目前简历以项目经历为主，但一旦补入实习经历后，HR注意力会转移至实习部分，现有项目条目数量过多会导致篇幅失衡，需主动删减重复项目。
  - action: 将实习经历加入后，删去内容相似的项目条目（如保留最有代表性的2个），让实习经历占据与项目经历同等甚至更大篇幅，突出真实工作经验。
...
- id=10782 score=10 role=procurement,supply_chain_logistics target=procurement_specialist,purchasing_agent,supply_chain_analyst tags=weak_experience_keyword_evidence,weak_result_orientation,vague_project_details
  - title: 求职者简历中采购相关经历描述过于笼统宽泛
  - action: 将采购经历拆分为独立bullet point，结构为：Manage procurement process for operations, including collecting purchase requirements, comparing multiple suppliers, selecting optimal vendors, and making purchase orders to enhance supply chain efficiency；同时补充历史数据参考（如去年支出40万、业务增长15%
...
- #9 seg_22876 [no_role_signal] role=universal target=universal
  - title: 求职者简历按时间线排列，将论文和专利放在靠前位置，但这些内容与目标岗位关联性不如技能/工具模块强，导致HR扫描时无法第一时间看到最匹配的信息。
  - action: 将简历中的 skills 和相关工具模块调整到靠前位置，paper 和 patent 移至后面；同时可在教育经历下补充与目标岗位相关的重要课程名称，以增强匹配度。
- #11 seg_1570 [role_mismatch, no_role_signal] role=machine_learning,data_analyst target=data_analyst
  - title: 求职者实际掌握feature engineering等技术技能，但简历项目描述中未体现这些关键词，导致ATS关键词匹配度低，250份投递仅获2次面试
  - action: 将feature engineering、EDA（exploratory data analysis）、data cleaning、data modeling、visualization等实际使用过的技术关键词补充进项目bullet point；按照实际操作流程（数据清洗→EDA→特征工程→模型选择→可视化）重新组织描述
...
- #9 seg_22876 [no_role_signal] role=universal target=universal
  - title: 求职者简历按时间线排列，将论文和专利放在靠前位置，但这些内容与目标岗位关联性不如技能/工具模块强，导致HR扫描时无法第一时间看到最匹配的信息。
  - action: 将简历中的 skills 和相关工具模块调整到靠前位置，paper 和 patent 移至后面；同时可在教育经历下补充与目标岗位相关的重要课程名称，以增强匹配度。
- #10 seg_1570 [role_mismatch, no_role_signal] role=machine_learning,data_analyst target=data_analyst
  - title: 求职者实际掌握feature engineering等技术技能，但简历项目描述中未体现这些关键词，导致ATS关键词匹配度低，250份投递仅获2次面试
  - action: 将feature engineering、EDA（exploratory data analysis）、data cleaning、data modeling、visualization等实际使用过的技术关键词补充进项目bullet point；按照实际操作流程（数据清洗→EDA→特征工程→模型选择→可视化）重新组织描述
- #11 seg_6807 [no_role_signal] role=universal target=universal
  - title: 求职者简历项目过多且偏长，部分项目与目标岗位（CDM相关）关联度低，稀释了简历的针对性和重点。
...
### Flagged Current Top Rows
- #8 seg_22419 [not_role_safe] role=universal target=universal
  - title: 求职者目前简历以项目经历为主，但一旦补入实习经历后，HR注意力会转移至实习部分，现有项目条目数量过多会导致篇幅失衡，需主动删减重复项目。
  - action: 将实习经历加入后，删去内容相似的项目条目（如保留最有代表性的2个），让实习经历占据与项目经历同等甚至更大篇幅，突出真实工作经验。
...
- #9 seg_22419 [not_role_safe] role=universal target=universal
  - title: 求职者目前简历以项目经历为主，但一旦补入实习经历后，HR注意力会转移至实习部分，现有项目条目数量过多会导致篇幅失衡，需主动删减重复项目。
  - action: 将实习经历加入后，删去内容相似的项目条目（如保留最有代表性的2个），让实习经历占据与项目经历同等甚至更大篇幅，突出真实工作经验。
- #10 seg_16332 [role_mismatch] role=data_analyst target=data_analyst
  - title: 求职者简历未针对不同目标公司的JD做定制化调整，关键词出现频率不足，难以通过ATS机筛，且工作经验部分未能体现与JD要求相关的经历。
  - action: 针对有兴趣的目标公司，逐一对照JD的bullet point，将JD中的关键词嵌入简历工作经验描述中，并在skill栏重复出现关键词2-3次，提升ATS匹配分数。
- #11 seg_15094 [role_mismatch] role=data_analyst target=data_analyst
  - title: 求职者有多个项目经历，需判断哪些与ESG/sustainability方向最相关，并确认哪些项目有空间植入技术工具（Excel、SQL等）以增强简历竞争力。
  - action: 从现有项目中优先保留与ESG/sustainability主题最相关、且时间较近（记忆清晰、细节可补充）的项目；对入选项目逐一评估可植入的技术工具关键词，确保每条bullet point体现实际使用的数据处理工具。
- #12 seg_2479 [role_mismatch] role=software_engineer,data_analyst target=universal
...
### Flagged Current Top Rows
- #8 seg_19346 [no_role_signal] role=universal target=universal
