...
- pass: true
- current: 简历与目标 JD 的关键词匹配偏低（当前 4/23），ATS 扫描时匹配信号不够强，容易在第一轮被过滤。
- action: 针对每个目标岗位定制简历，确保每条描述尽量与该岗位JD关键词和需求高度匹配；删除或弱化与目标岗位无关的泛化内容，让HR一眼看出你背景与岗位高度契合
- mentor: HR筛选简历时无暇深挖细节，简历越具体、与JD越匹配，HR越容易做出给面试的决定；反之，若简历显示你'什么都沾一点'，HR会认为其专注度不足，每个岗位都会损失机会。
