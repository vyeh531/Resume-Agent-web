...
## 12. 补上 JD 中真实掌握的岗位关键词
- id: fb_jd_keyword_alignment
- source: fallback
- family/depth: jd_keyword_alignment / evidence
- mode: raw
- pass: true
- current: 当前简历缺少目标岗位会搜索的核心硬技能和工具词，导致 JD Match 分数偏低。
- action: 把你真实掌握的 Management Trainee 相关关键词补进 Skills，例如 department rotation、operations、sales、finance、customer service、data analysis、reporting、process improvement、communication 或 leadership potential。
- mentor: ATS 做关键词匹配时，会扫全文搜索 JD 中的核心技能词。如果 Skills 区块缺少这些词，系统会把你的匹配分打低，recruiter 在关键词筛选时也会跳过你。
- HR: HR 在筛简历时会用 JD 技能词快速确认你是否进入下一轮；缺少核心关键词会让你看起来不像这份 JD 的直接匹配人选。
