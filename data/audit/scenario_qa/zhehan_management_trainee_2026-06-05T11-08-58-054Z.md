...
## 9. 补齐 JD 缺失关键词
- id: fb_obligation_problemlow_jd_keyword_match
- source: fallback
- family/depth: summary_positioning / evidence
- mode: raw
- pass: true
- current: 简历和目标 JD 的语言匹配还不够，ATS 可能无法快速识别你和这个岗位的直接关联。
- action: 对照 JD 标出真实掌握的核心工具、技能和职责词，把它们分别写进 Summary、Skills 和最相关的 Experience bullet。
