...
## 8. 基本资料完整性优化建议
- id: fb_obligation_problemweak_summary_role_alignment
- source: fallback
- family/depth: summary_positioning / evidence
- mode: raw
- pass: true
- current: Summary 和目标岗位的关联不够直接，HR 初筛时可能看不出你正在投这个方向。
- action: 围绕 Management Trainee 重新检查 summary 部分，把最相关的关键词、职责和结果证据写得更明确。
- mentor: 这条建议来自 ATS 对简历维度的综合诊断，重点是把问题转化成可以直接修改的简历动作。
- HR: 
