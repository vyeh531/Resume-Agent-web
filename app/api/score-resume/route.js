...
export async function POST(request) {
  try {
    const { resumeText, jobTitle, jdText, fileName = '' } = await request.json();
    if (!resumeText) return Response.json({ error: 'resumeText is required' }, { status: 400 });

    const scoringJobTitle = String(jobTitle || '').trim();
    const result = scoreResumeSystem(resumeText, scoringJobTitle, jdText, { fileName });
    let sessionId = null;
