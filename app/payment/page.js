  ...
  return (
    <>
      <style>{`
        ...
        .pay-summary .item{display:flex;justify-content:space-between;gap:12px;padding:4px 0;color:var(--ink)}
        ...
        .qr{width:140px;height:140px;border:2px solid var(--line);border-radius:var(--r-md);margin:0 auto;background:var(--paper-deep);display:grid;place-items:center;font-family:var(--mono);font-size:24px;font-weight:700;color:var(--ink-soft)}
      `}</style>
      ...
      <div className="page payment-page">
        ...
        <div className="pay-card fade-in">
          <div className="pay-summary">
            ...
            <div className="item"><span>4 位行业导师完整建议</span><span className="text-mute">最多 12 条</span></div>
          ...
          <div className="text-mute pay-amount-label" style={{fontSize:'11px',fontFamily:'var(--mono)',letterSpacing:'.12em',textTransform:'uppercase'}}>应付金额</div>
          ...
          <div style={{margin:'26px 0 12px'}}><div className="qr">QR</div></div>
          <p className="text-mute pay-scan" style={{fontSize:'12px',margin:'0 0 6px'}}>微信扫码完成支付</p>
        ...
        <div className="pay-foot">
          支付遇到问题？联系客服 <a>edaix-support</a><br/>
          本订单由 EdAIX 提供
        </div>
      </div>
      ...
      <Script id="payment-logic" strategy="afterInteractive">{`
        ...
        function paymentLocale() {
          try {
            const store = JSON.parse(localStorage.getItem("resumeFixMVP") || "{}");
            return String(store.locale || document.documentElement.lang || "zh-CN").toLowerCase().startsWith("en") ? "en-US" : "zh-CN";
          } catch {
            return "zh-CN";
          }
        }
        ...
        function renderTimer(expired) {
          if (!el) return;
          const isEn = paymentLocale() === "en-US";
          if (expired) {
            el.textContent = isEn ? "QR code expired. Please refresh the page." : "二维码已过期，请刷新页面";
            return;
          }
          const m = Math.floor(timer / 60), s = timer % 60;
          el.textContent = isEn
            ? "QR code expires in " + m + ":" + String(s).padStart(2, "0")
            : "二维码 " + m + ":" + String(s).padStart(2, "0") + " 后过期";
        }
        renderTimer(false);
        const countdown = setInterval(() => {
          if (timer <= 0) { clearInterval(countdown); renderTimer(true); return; }
          timer--;
          renderTimer(false);
        }, 1000);
      `}</Script>
